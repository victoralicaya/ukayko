...
function zen_get_banner_data_recent($banner_id, $days) {
	...
	where wtwbmbh_id = '" . (int)$banner_id . "'
...
function zen_get_banner_data_yearly($banner_id) {
	...
	$years = array(0=>'');
	$stats = array();
	$result = $db->Execute("select year(wtwbmbh_history_date) as year,
	sum(wtwbmbh_shown) as impressions, sum(wtwbmbh_clicked) as clicks
	from " . TABLE_WT_SLIDESHOW_MANAGER_BANNERS_HISTORY . "
	where wtwbmbh_id = '" . (int)$banner_id . "'
...
function zen_get_banner_data_monthly($banner_id, $year = '') {
	...
	$result = $db->Execute("select month(wtwbmbh_history_date) as banner_month, sum(wtwbmbh_shown) as impressions,
	sum(wtwbmbh_clicked) as clicks
	from " . TABLE_WT_SLIDESHOW_MANAGER_BANNERS_HISTORY . "
	where wtwbmbh_id = '" . (int)$banner_id . "'
	and year(wtwbmbh_history_date) = '" . zen_db_input($year) . "'
...
function zen_get_banner_data_daily($banner_id, $year = '', $month = '') {
	...
	$result = $db->Execute("select dayofmonth(wtwbmbh_history_date) as banner_day,
	wtwbmbh_shown as impressions, wtwbmbh_clicked as clicks
	from " . TABLE_WT_SLIDESHOW_MANAGER_BANNERS_HISTORY . "
	where wtwbmbh_id = '" . (int)$banner_id . "'
	and month(wtwbmbh_history_date) = '" . zen_db_input($month) . "'
	and year(wtwbmbh_history_date) = '" . zen_db_input($year) . "' order by banner_day");
