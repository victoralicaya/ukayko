...
function wt_neoncart_exe_datafile(){
...
function wt_neoncart_executeSq($sql_file, $database, $table_prefix = '', $isupgrade=false) {
  $debug=false;
...
//echo '*}'.$line.'<br>';
          ...
          case (substr($line_upper, 0, 11) == 'DROP TABLE ' && $param[2] != 'IF'):
            if (!$checkprivs = wt_neoncart_check_database_privs('DROP')) $result=sprintf(REASON_NO_PRIVILEGES,'DROP');
            if (!wt_neoncart_table_exists($param[2]) || zen_not_null($result)) {
              wt_neoncart_write_to_upgrade_exceptions_table($line, (zen_not_null($result) ? $result : sprintf(REASON_TABLE_DOESNT_EXIST,$param[2])), $sql_file);
              $ignore_line=true;
          ...
          case (substr($line_upper, 0, 13) == 'CREATE TABLE '):
            ...
            $result=wt_neoncart_table_exists($table);
            if ($result==true) {
              $ignore_line=true;
              if (strtoupper($param[2].' '.$param[3].' '.$param[4]) != 'IF NOT EXISTS') {
                wt_neoncart_write_to_upgrade_exceptions_table($line, sprintf(REASON_TABLE_ALREADY_EXISTS,$table), $sql_file);
          ...
          case (substr($line_upper, 0, 13) == 'REPLACE INTO '):
            ...
            if (!$tbl_exists = wt_neoncart_table_exists($param[2])) $result=sprintf(REASON_TABLE_NOT_FOUND,$param[2]).' CHECK PREFIXES!';
            // check to see if INSERT command may be safely executed for "configuration" or "product_type_layout" tables
            if (($param[2]=='configuration'       && ($result=wt_neoncart_check_config_key($line))) or
                ($param[2]=='product_type_layout' && ($result=wt_neoncart_check_product_type_layout_key($line))) or
                ($param[2]=='configuration_group' && ($result=wt_neoncart_check_cfggroup_key($line))) or
                (!$tbl_exists)    ) {
              wt_neoncart_write_to_upgrade_exceptions_table($line, $result, $sql_file);
              $ignore_line=true;
              break;
            } else {
          ...
          case (substr($line_upper, 0, 19) == 'INSERT IGNORE INTO '):
            //check to see if table prefix is going to match
            if (!$tbl_exists = wt_neoncart_table_exists($param[3])) {
            ...
            if ($result=wt_neoncart_check_alter_command($param)) {
              wt_neoncart_write_to_upgrade_exceptions_table($line, $result, $sql_file);
              $ignore_line=true;
              break;
            } else {
          ...
          case (substr($line_upper, 0, 15) == 'TRUNCATE TABLE '):
            ...
            if (!$tbl_exists = wt_neoncart_table_exists($param[2])) {
              $result=sprintf(REASON_TABLE_NOT_FOUND,$param[3]).' CHECK PREFIXES!';
              wt_neoncart_write_to_upgrade_exceptions_table($line, $result, $sql_file);
              $ignore_line=true;
              break;
          ...
          case (substr($line_upper, 0, 13) == 'RENAME TABLE '):
            ...
            if (isset($param[3]) && $param[3] != '') {
              wt_neoncart_write_to_upgrade_exceptions_table($line, 'RENAME TABLE command must be split onto 2 rows for proper parsing.  Or use phpMyAdmin instead.', $sql_file);
              $result=sprintf('RENAME TABLE [%s] command must be split onto 2 rows for proper parsing.',$param[2]).' CHECK PREFIXES!';
              $ignore_line=true;
            }
            //check to see if table prefix is going to match
            if (!$tbl_exists = wt_neoncart_table_exists($param[2])) {
              wt_neoncart_write_to_upgrade_exceptions_table($line, sprintf(REASON_TABLE_NOT_FOUND,$param[2]).' CHECK PREFIXES!', $sql_file);
          ...
          case (substr($line_upper, 0, 3) == 'TO '):
            if (!isset($param[1]) || $param[1] == '') {
              wt_neoncart_write_to_upgrade_exceptions_table($line, 'RENAME TABLE command must be split onto 2 rows (with TO clause on 2nd line) for proper parsing.  Or use phpMyAdmin instead.', $sql_file);
          ...
          case (substr($line_upper, 0, 7) == 'UPDATE '):
            //check to see if table prefix is going to match
            if (!$tbl_exists = wt_neoncart_table_exists($param[1])) {
              wt_neoncart_write_to_upgrade_exceptions_table($line, sprintf(REASON_TABLE_NOT_FOUND,$param[1]).' CHECK PREFIXES!', $sql_file);
          ...
          case (substr($line_upper, 0, 11) == 'DROP INDEX '):
            ...
            if ($result=wt_neoncart_drop_index_command($param)) {
...
//               wt_neoncart_write_to_upgrade_exceptions_table($line, $result, $sql_file);
          ...
          case (substr($line_upper, 0, 13) == 'CREATE INDEX ' || (strtoupper($param[0])=='CREATE' && strtoupper($param[2])=='INDEX')):
            ...
            if ($result=wt_neoncart_create_index_command($param)) {
              wt_neoncart_write_to_upgrade_exceptions_table($line, $result, $sql_file);
              $ignore_line=true;
              break;
            } else {
...
//        } // endif $table_prefix
        ...
        if ($complete_line) {
          ...
          if (zen_not_null($result) && !wt_neoncart_check_exceptions($result, $line) ) $errors[]=$result;
          // reset var's
          $newline = '';
          $keep_together=1;
          $complete_line = false;
          if ($ignore_line && !wt_neoncart_check_exceptions($result, $line)) $ignored_count++;
          $ignore_line=false;
  ...
  function wt_neoncart_check_exceptions($result, $line) {
  ...
  function wt_neoncart_table_exists($tablename, $pre_install=false) {
    global $db;
  ...
  function wt_neoncart_check_database_privs($priv='',$table='',$show_privs=false) {
  ...
  function wt_neoncart_drop_index_command($param) {
    if (!$checkprivs = wt_neoncart_check_database_privs('INDEX')) return sprintf(REASON_NO_PRIVILEGES,'INDEX');
...
//        if (!$checkprivs = wt_neoncart_check_database_privs('INDEX')) return sprintf(REASON_NO_PRIVILEGES,'INDEX');
  ...
  function wt_neoncart_create_index_command($param) {
 ...
 */
  ...
  function wt_neoncart_check_alter_command($param) {
    global $db;
    if (!zen_not_null($param)) return "Empty SQL Statement";
    if (!$checkprivs = wt_neoncart_check_database_privs('ALTER')) return sprintf(REASON_NO_PRIVILEGES,'ALTER');
 ...
 */
  ...
  function wt_neoncart_check_config_key($line) {
    global $db;
    $values=array();
    $values=explode("'",$line);
  ...
  function wt_neoncart_check_product_type_layout_key($line) {
    global $db;
    $values=array();
    $values=explode("'",$line);
    $title = $values[1];
  ...
  function wt_neoncart_check_cfggroup_key($line) {
    global $db;
    $values=array();
    $values=explode("'",$line);
    $id = $values[1];
  ...
  function wt_neoncart_write_to_upgrade_exceptions_table($line, $reason, $sql_file) {
    global $db;
    wt_neoncart_create_exceptions_table();
  ...
  function wt_neoncart_purge_exceptions_table() {
    global $db;
    wt_neoncart_create_exceptions_table();
  ...
  function wt_neoncart_create_exceptions_table() {
    global $db;
    if (!wt_neoncart_table_exists(TABLE_UPGRADE_EXCEPTIONS)) {
    ...
    switch ($action) {
      case 'execute':
       if (isset($_POST['query_string']) && $_POST['query_string'] !='' ) {
         ...
         $query_results = wt_neoncart_executeSq($query_string, DB_DATABASE, DB_PREFIX);
...
function wt_neoncart_execute_sql($sql_file_path,$dbname,$dbprefix){
	global $messageStack,$db;
	wt_neoncart_exe_datafile();
	$query_results = wt_neoncart_executeSq($sql_file_path, $dbname, $dbprefix);
	if($query_results['queries'] > 0 && $query_results['queries'] != $query_results['ignored']){
		$messageStack->add($query_results['queries']. ' statements processed.', 'success');
		$messageStack->add_session('Template has been installed successfully.', 'success');
...
function wt_neoncart_execute_sql_force($sql_file_path,$dbname,$dbprefix){
	global $messageStack,$db;
	wt_neoncart_exe_datafile();
	$query_results = wt_neoncart_executeSq($sql_file_path, $dbname, $dbprefix);
	if($query_results['queries'] > 0 && $query_results['queries'] != $query_results['ignored']){
...
function wt_neoncart_checkdb_tables($template_tables){
	$template_tables=explode(",",$template_tables);
	$table_exits=array();
	foreach($template_tables as $k=>$v){
		if(wt_neoncart_checkdbtable($v)==1){
			$table_exits[]=$v;
		}
	}
	return $table_exits;
...
function wt_neoncart_checkdbtable($table){
	global $db;
