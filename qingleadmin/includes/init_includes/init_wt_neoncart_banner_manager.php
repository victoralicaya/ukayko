...
if (!empty($_SESSION['admin_id']) && $_SERVER['SCRIPT_NAME'] !== DIR_WS_ADMIN . (!strstr(FILENAME_LOGIN, '.php') ? FILENAME_LOGIN . '.php' : FILENAME_LOGIN) && $_SERVER['SCRIPT_NAME'] !== DIR_WS_ADMIN . (!strstr(FILENAME_LOGOFF, '.php') ? FILENAME_LOGOFF . '.php' : FILENAME_LOGOFF)) {
	$module_constant = 'WT_NEONCART_BANNER_MANAGER'; // This should be a UNIQUE name followed by _VERSION for convention
	$module_installer_directory = DIR_FS_ADMIN . 'includes/installers/wt_neoncart_banner_manager'; // This is the directory your installer is in, usually this is lower case
	$module_name = "WT Neoncart Banner Manager"; // This should be a plain English or Other in a user friendly way
	$admin_page = 'WTNeoncartBannerManager';
