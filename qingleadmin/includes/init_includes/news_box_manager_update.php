...
if (version_compare($nb_current_version, '3.0.0', '<')) {
    ...
    $db->Execute(
        "INSERT IGNORE INTO " . TABLE_CONFIGURATION . " 
            (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added, use_function, set_function) 
         VALUES 
            ('All Articles: Display Mode', 'NEWS_BOX_ALL_ARTICLES_DISPLAY', 'Listing', 'Choose the format used to display the articles on the <code>all_articles</code> page.  The <em>Table</em> format displays the most recent articles in a table-type format.  The <em>Listing</em> format displays the most recent articles in a listing format.', $cgi, 405, now(), NULL, 'zen_cfg_select_option(array(\'Table\', \'Listing\'),')"
    );
	
	/* BOF--WT--NEWSBOX --10-02-23 */
	$db->Execute(
        "INSERT IGNORE INTO " . TABLE_CONFIGURATION . " 
            (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added, use_function, set_function) 
         VALUES 
            ('News Box Home Image Width', 'NEWS_BOX_HOME_IMAGE_WIDTH', '380', 'Set home page news box image width. default:380', $cgi, 410, now(), NULL, '');"
    );
	$db->Execute(
        "INSERT IGNORE INTO " . TABLE_CONFIGURATION . " 
            (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added, use_function, set_function) 
         VALUES 
            ('News Box Home Image Height', 'NEWS_BOX_HOME_IMAGE_HEIGHT', '', 'Set home page news box image height. default:blank space', $cgi, 420, now(), NULL, '');"
    );
	/* EOF--WT--NEWSBOX --10-02-23 */
