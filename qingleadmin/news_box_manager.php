...
switch ($action) {
    ...
    case 'update':
		...
		if(!empty( $_POST['news_image'] )){
			$sql_data_array['news_image'] = $_POST['news_image'];
		}
		/* EOF--WT-- NEWS-IMAGE*/
    ...
    case 'updateedit':
        ...
        if ($action == 'newpreview' || $action == 'updatepreview') { 
			...
			/* BOF--WT-- NEWS-IMAGE*/
			$news_image_dir_name = 'news';
			$news_image_target = DIR_FS_CATALOG_IMAGES . $news_image_dir_name . '/';
			$news_image = zen_db_prepare_input($_FILES['news_image']['name']);
			
			if($news_image){
				if (!file_exists($news_image_target)) {
					mkdir($news_image_target, 0777, true);
				}
				$news_image = new upload('news_image');
				$news_image->set_extensions(array('jpg', 'jpeg', 'gif', 'png', 'webp', 'flv', 'webm', 'ogg'));
				$news_image->set_destination($news_image_target);
				if (($news_image->parse() == false) || ($news_image->save() == false)) {
					$messageStack->add(ERROR_BANNER_IMAGE_REQUIRED, 'error');
				}
			}
			
			if($news_image->filename){
				$chk_col = $db->Execute('SHOW COLUMNS FROM '.TABLE_BOX_NEWS.' LIKE "news_image"');
				if($chk_col->RecordCount() == 0){
					$db->Execute('ALTER TABLE '.TABLE_BOX_NEWS.' ADD COLUMN news_image TEXT NULL AFTER news_end_date;');
				}
				$db_image_location = $news_image_dir_name.'/'.$news_image->filename;
				$_POST['news_image'] = $db_image_location;
			}
			/* EOF--WT-- NEWS-IMAGE*/
...
?>
		...
		<?php /* BOF--WT-- NEWS-IMAGE*/ ?>
		<div class="form-group">
            <?php echo zen_draw_label(TEXT_NEWS_IMAGE, 'news_image', 'class="col-sm-3 control-label"'); ?>
            <div class="col-sm-9 col-md-6">
                <?php echo zen_draw_file_field('news_image', '', 'class="form-control"'); ?>
				<?php 
					if($nInfo->news_image){
						echo zen_image(DIR_WS_CATALOG_IMAGES . $nInfo->news_image, $nInfo->news_image, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, '');
					}
				?>
            </div>
        </div>
		<?php /* EOF--WT-- NEWS-IMAGE*/ ?>
<?php 
...
//
    ...
    if ($action == 'previewonly') {
		...
		/* BOF--WT-- NEWS-IMAGE*/
        $submit_buttons .= zen_draw_hidden_field('news_image', $nInfo->news_image);
		/* BOF--WT-- NEWS-IMAGE*/
...
?>
	...
	<?php /* BOF--WT-- NEWS-IMAGE*/ ?>
	<div class="row large">
		<div class="col-sm-1"><strong><?php echo TEXT_NEWS_IMAGE; ?></strong></div>
        <div class="col-sm-5"><?php echo (empty($nInfo->news_image)) ? '' : zen_image(DIR_WS_CATALOG_IMAGES . $nInfo->news_image, $nInfo->news_image, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, ''); ?></div>
	</div>
	<?php /* EOF--WT-- NEWS-IMAGE*/ ?>
    <hr />
<?php
...
?>
					...
					<?php /* BOF--WT-- NEWS-IMAGE*/ ?>
					<td class="text-center"><?php echo TABLE_HEADING_IMAGE; ?></td>
					<?php /* BOF--WT-- NEWS-IMAGE*/ ?>
...
<?php
	...
	/* BOF--WT-- NEWS-IMAGE*/
    $news_query_raw = 
        "SELECT n.box_news_id, nc.news_title, nc.news_content, n.news_added_date, n.news_modified_date, n.news_start_date, n.news_end_date, n.news_status, n.news_content_type, n.news_image 
...
?>
				...
				<?php 
					if($news->fields['news_image']){
						echo zen_image(DIR_WS_CATALOG_IMAGES . $news->fields['news_image'], $news->fields['news_image'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, '');
					}
				?>
				</td>
				<?php /* EOF--WT-- NEWS-IMAGE*/ ?>
