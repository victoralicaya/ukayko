...
//@TODO        $this->notify('NOTIFY_?LOGIN_ATTEMPT', null, $is_logged_in);
    ...
    public function getMyListings(int $max_number_to_return = 0, &$returned_listings_split = null): array
    {
        //$debug = true;
        $language = $_SESSION['languages_id'];
        global $db;

        $sqlListings = "SELECT p.products_type, p.products_id, pd.products_name, p.products_quantity,
                                      p.products_price, p.products_status, p.products_model, p.products_sort_order, p.products_price_sorter, p.products_weight,
                                      p.master_categories_id";
        $sqlListings .= " FROM " . TABLE_PRODUCTS . " p";
        $sqlListings .= " LEFT JOIN " . TABLE_PRODUCTS_DESCRIPTION . " pd ON (pd.products_id = p.products_id)";
        $sqlListings .= " LEFT JOIN " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c ON (p2c.products_id = p.products_id) ";

        $where = " WHERE pd.language_id = :languageID AND p.seller_id = :customerID";
        $order_by = " ORDER BY p.products_id DESC";
        $sqlListings .= $where . $order_by;

        $sqlListings = $db->bindVars($sqlListings, ':customerID', $this->customer_id, 'integer');
        $sqlListings = $db->bindVars($sqlListings, ':languageID', $language, 'integer');
        //echo $sqlListings . "<br><br>";

        $listings_split = new splitPageResults($sqlListings, $max_number_to_return);
        //echo $sqlListings;
        if ($returned_listings_split !== null) {
            $returned_listings_split = $listings_split;
        }
        $results = $db->Execute($listings_split->sql_query);
        //var_dump($results);
        $listingsArray = [];
        foreach ($results as $result) {
            $listingsArray[] = [
                'products_id' => (int)$result['products_id'],
                'products_name' => $result['products_name'],
                'products_model' => $result['products_model'],
                'products_price' => $result['products_price'],
                'products_quantity' => $result['products_quantity'],
                'products_status' => $result['products_status'],
                'products_type' => $result['products_type'],
            ];
        }
