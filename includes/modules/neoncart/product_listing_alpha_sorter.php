...
// build alpha sorter dropdown
  if (PRODUCT_LIST_ALPHA_SORTER == 'true') {
	...
	if (TEXT_PRODUCTS_LISTING_ALPHA_SORTER != '') {
        echo '<label class="inputLabel select-label d-none d-inline" for="select-alpha_filter_id">' . TEXT_PRODUCTS_LISTING_ALPHA_SORTER . '</label>';
    } else {
        echo '<label class="inputLabel sr-only select-label d-none d-inline" for="select-alpha_filter_id">' . TEXT_PRODUCTS_LISTING_ALPHA_SORTER_NAMES . '</label>';
    }
	echo '<div class="select-wrapper-sm d-inline-block">' . zen_draw_pull_down_menu('alpha_filter_id', $letters_list, (isset($_GET['alpha_filter_id']) ? $_GET['alpha_filter_id'] : ''), 'onchange="this.form.submit()"').'</div>';
  }
