...
$max_display_products = ( isset( $max_products ) ) ? $max_products : MAX_DISPLAY_TOP_RATED_PRODUCTS;
$max_display_columns = ( isset( $show_rows ) ) ? $show_rows : SHOW_PRODUCT_INFO_COLUMNS_TOP_RATED_PRODUCTS;
...
if ( (($manufacturers_id > 0 && empty($_GET['filter_id'])) || !empty($_GET['music_genre_id']) || !empty($_GET['record_company_id'])) || empty($products_tab_category_id) ) {
	$products_tabl_query = "SELECT DISTINCT p.products_id, p.products_image, p.products_tax_class_id, pd.products_name,
	p.products_date_added, p.products_date_available, p.products_price, p.products_type, p.master_categories_id, p.products_quantity, p.product_is_call, p.products_ordered, f.featured_id, s.specials_id, r.reviews_rating
	FROM " . TABLE_REVIEWS . " r
	JOIN " . TABLE_PRODUCTS . " p ON r.products_id = p.products_id
	...
	WHERE p.products_status = 1 order by r.reviews_rating DESC";
	} else {
	...
	if (is_array($productsInCategory) && sizeof($productsInCategory) > 0) {
		...
		AND p.products_id in (" . $list_of_products . ") order by r.reviews_rating desc";
	}
}

if ($products_tabl_query != '') $products_tab = $db->ExecuteRandomMulti($products_tabl_query, $max_display_products);
...
if ($num_products_count > 0) {
	if ($num_products_count < SHOW_PRODUCT_INFO_COLUMNS_TOP_RATED_PRODUCTS || SHOW_PRODUCT_INFO_COLUMNS_TOP_RATED_PRODUCTS == 0 ) {
		$col_width = floor(100/$num_products_count);
		} else {
		$col_width = floor(100/SHOW_PRODUCT_INFO_COLUMNS_TOP_RATED_PRODUCTS);
	}
