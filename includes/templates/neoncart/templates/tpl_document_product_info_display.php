 ...
 */
?>
<div class="centerColumn product-shop" id="docProductDisplay">
					...
					<?php
						if ($products_date_available > date('Y-m-d H:i:s')) {
							if ($flag_show_product_info_date_available == 1) {?>
								<li id="productDateAvailable" class="docProduct product-date-availability"><?php echo sprintf(TEXT_DATE_AVAILABLE, '<strong>' . zen_date_long($products_date_available) . '</strong>'); ?></li>
							<?php }
						} else {
							if ($flag_show_product_info_date_added == 1) { ?>
								<li id="productDateAdded" class="docProduct product-date-availability"><?php echo sprintf(TEXT_DATE_ADDED,  '<strong>' . zen_date_long($products_date_added)  . '</strong>' ); ?></li>
		...
		<dl id="collateral-tabs" class="tab-content collateral-tabs">
			...
			<dd class="tab tab-pane active in" id="Description">
				<div class="tab-description">
					<div id="productDescription" class="docProduct biggerText"><p class="text"><?php echo stripslashes($products_description); ?></p></div>
					<!--bof Product URL -->
					<?php
