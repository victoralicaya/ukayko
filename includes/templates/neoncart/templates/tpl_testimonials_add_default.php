...
<div class="centerColumn" id="testimonialDefault">
  <h1 class="tt-title no-border text-left"><?php echo HEADING_ADD_TITLE; ?></h1>
  <?php echo zen_draw_form('new_testimonial', zen_href_link(FILENAME_TESTIMONIALS_ADD, 'action=send', 'SSL')); ?>

  <?php
    if (isset($_GET['action']) && ($_GET['action'] == 'success')) {
  ?>
  <div class="mainContent alert alert-success success"><?php echo TESTIMONIAL_SUCCESS; ?></div>
  <?php
    } else {
  ?>
  ...
  <div class="tt-card-box">
    ...
    <div class="alert alert-info forward mb-3"><?php echo zen_image($template->get_template_dir(RETURN_REQUIRED_IMAGE, DIR_WS_TEMPLATE, $current_page_base,'images'). '/' . RETURN_REQUIRED_IMAGE, RETURN_REQUIRED_IMAGE_ALT, RETURN_REQUIRED_IMAGE_WIDTH, RETURN_REQUIRED_IMAGE_HEIGHT) . RETURN_REQUIRED_INFORMATION . zen_image($template->get_template_dir(RETURN_OPTIONAL_IMAGE, DIR_WS_TEMPLATE, $current_page_base,'images'). '/' . RETURN_OPTIONAL_IMAGE, RETURN_OPTIONAL_IMAGE_ALT, RETURN_OPTIONAL_IMAGE_WIDTH, RETURN_OPTIONAL_IMAGE_HEIGHT) . RETURN_OPTIONAL_INFORMATION; ?></div>
    <fieldset id="personal">
      <h4 class="tt-title"><?php echo TESTIMONIAL_CONTACT; ?></h4>
    ...
    <hr>
    <fieldset id="write">
      <h4 class="tt-title"><?php echo TEXT_TESTIMONIALS_HTML_TEXT ; ?></h4>
