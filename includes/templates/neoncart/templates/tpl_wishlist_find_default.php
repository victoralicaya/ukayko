<div id="wishlist" class="wishlist"> <!-- begin wishlist id for styling -->
	<h1 class="tt-title noborder text-left"><?php echo TEXT_WISHLIST_FOR . '<b>'.$customers_name.'</b>'; ?></h1>
    <!-- control -->
	...
	<div class="sorter filter-row d-flex gap-2">
		...
		<div class="wishlist<?php echo (!un_is_empty($tdclass)? '-'.$tdclass: ''); ?>">
			...
			<?php
            	$the_button = '<a href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $products->fields['products_id'], 'SSL') . '" title="'.BUTTON_IN_CART_ALT.'">' . zen_image_button(BUTTON_IMAGE_IN_CART, BUTTON_IN_CART_ALT) . '</a>';
