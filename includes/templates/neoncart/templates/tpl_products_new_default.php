 ...
 */
?>
<div class="centerColumn products-in-listing" id="newProductsDefault">

<h1 class="tt-title" id="newProductsDefaultHeading"><?php echo HEADING_TITLE; ?></h1>
<?php
/********************************** GRID LIST VIEW ***************************************/
  if ( defined( 'PRODUCT_LISTING_LAYOUT_STYLE_CUSTOMER' ) && PRODUCT_LISTING_LAYOUT_STYLE_CUSTOMER == '1' ) {
      $gridlist_tab = wt_neoncart_gridlist(FILENAME_PRODUCTS_NEW);
  }
  /**********************************EOF GRID LIST VIEW ***************************************/
?>
<?php
...
?>
  <div class="tt-pagination tt-pagination-left pagi-top mt-3">
    <div id="newProductsDefaultListingTopNumber" class="navSplitPagesResult back"><?php echo $products_new_split->display_count(TEXT_DISPLAY_NUMBER_OF_PRODUCTS_NEW); ?>
    </div>
...
?>
  ...
  ?>
    ...
    <div id="newProductsDefaultListingBottomLinks" class="navSplitPagesLinks forward">
      <?php echo TEXT_RESULT_PAGE . $products_new_split->display_links($max_display_page_links, zen_get_all_get_params(array('page', 'info', 'x', 'y', 'main_page')), $paginateAsUL); ?>
    </div>
  <?php
    }
  ?>
  ...
  <?php
    ...
    // only show when there is something to submit
  ?>
    <div class="buttonRow forward"><?php echo zen_image_submit(BUTTON_IMAGE_ADD_PRODUCTS_TO_CART, BUTTON_ADD_PRODUCTS_TO_CART_ALT, 'id="submit2" name="submit1"'); ?>
    </div>
  <?php
